...
from src.Medvit import MedViT_small
...
print(torch.cuda.memory_allocated())  # Print memory allocated
...
net = MedViT_small(num_classes=2)
net.to(device0)
...
save_path = f'Medvit_weights/{dataset_name}/{model_name}_{save_path_time}_Net.pth'
...
for epoch in range(epochs):
    ...
    for idx, (data, target) in enumerate(tqdm(train_bar)):
        ...
        label = target.squeeze(dim=1).long().to(device0)  # Use .squeeze() to flatten target to shape (batch_size,)
        optimizer.zero_grad()
        # Use autocast to enable mixed-precision forward pass
        with autocast():
            y_pred = net(img)
            loss = loss_function(y_pred, label)
        # Backpropagation with gradient scaling to prevent underflow
        ...
        scaler.update()
        # Collect predicted probabilities for AUC, to save memory on GPU
    ...
    torch.cuda.empty_cache()  # Clear cache after each epoch to prevent fragmentation
    ...
    with torch.no_grad():
        for idx, (data, target) in enumerate(tqdm(validate_loader)):
            img = data.float().to(device0)
            label = target.squeeze(dim=1).long().to(device0)
        ...
        # Optionally clear GPU cache after validation
        torch.cuda.empty_cache()  # Clear cache after validation loop
...
filename = f"Medvit_results/training_results_{model_name}_{result_time}.txt"
...
plt.legend()
plt.tight_layout()
plt.savefig(f"Medvit_results/Accuracy & AUC_{model_name}_{save_path_time}.png")  # Save to host server
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig(f"Medvit_results/Sensitivity and specificity_{model_name}_{save_path_time}.png")  # Save to host server
plt.close()
...
plt.tight_layout()
plt.legend()
plt.savefig(f"Medvit_results/Loss & F1_{model_name}_{save_path_time}.png")  # Save to host server
plt.close()
